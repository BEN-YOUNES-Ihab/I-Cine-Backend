...
export class MoviesController {
  ...
  getMoviesListbyCategory(
    @Query() movieCategoryFilterDto: movieCategoryFilterDto,
  ) {
  ...
  uploadMovieImage(
    @UploadedFiles()
    files: { file?: Express.Multer.File[]; secondFile?: Express.Multer.File[] },
    @Param('id', ParseIntPipe) movieId: number,
  ) {
    if (files.file == null && files.secondFile == null) {
      throw new NotFoundException('No files');
    } else if (files.file == null) {
      return this.movieService.uploadMovieBaniereImage(
        movieId,
        files.secondFile[0],
      );
    ...
    return this.movieService.uploadMovieImages(
      movieId,
      files.file[0],
      files.secondFile[0],
    );
