...
export class MoviesService {
  ...
  async uploadMovieImage(movieId: number, file: Express.Multer.File) {
    ...
    try {
      ...
      await this.cloudinaryService.deleteFile(
        movie_before.imageCloudinaryPublicId,
      );
    } catch (e) {
      console.log(e);
    }
  ...
  async uploadMovieImages(
    ...
    secondFile: Express.Multer.File,
  ) {
    if (!file || !secondFile) {
    ...
    try {
      ...
      await this.cloudinaryService.deleteFile(
        movie_before.baniereImageCloudinaryPublicId,
      );
    } catch (e) {
      console.log(e);
    }
    const uploadedFile = await this.cloudinaryService.uploadFile(file);
    const uploadedSecondFile =
      await this.cloudinaryService.uploadFile(secondFile);
    const movie = await this.prismaService.movie.update({
      where: { id: movieId },
      data: {
        imageUrl: uploadedFile.url,
