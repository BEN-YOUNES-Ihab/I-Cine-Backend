...
export class SessionsService {
  ...
  async getSessionsByMovieIdUser(sessionFilterDto: FilterUserDto) {
    ...
    if (date) {
      ...
      const filterMaxDate = new Date(newDate.setHours(23, 59, 59, 999));
  ...
  async getSessionsByMovieId(sessionFilterDto: FilterDto) {
    ...
    if (minDate) {
      ...
      const filterMinDate = new Date(newMinDate.setHours(0, 0, 0, 0));
      const filterMaxDate = new Date(newMaxDate.setHours(23, 59, 59, 10));

      where['OR'] = [{ date: { gte: filterMinDate, lte: filterMaxDate } }];
    }
  ...
  async updateSession(id: number, dto: updateSessionFormDto) {
    ...
    const sessionTest = await this.prismaService.session.findUnique({
      where: { id: id },
      include: { orders: true },
    });
